...
- **`oxc-walker: could not resolve a parseSync implementation`** (rencontré en essayant de lancer l'app avec le Node système v20.15.0) : `oxc-walker` (dépendance de Nuxt, utilisée pour analyser statiquement des fichiers comme `nuxt-auth-utils/.../session.js`) résout `parseSync` via un `require()` **synchrone** sur `oxc-parser` ou `rolldown/utils` — or ces deux paquets sont publiés en ESM pur (`"type": "module"`). Un `require()` sur un module ESM ne fonctionne que sous Node ≥20.19/≥22.12 (support natif `require(esm)`) ; sous Node 20.15, il échoue systématiquement, d'où l'erreur. **C'est exactement le même problème de version Node que celui déjà documenté plus haut**, qui se manifeste ici au runtime plutôt qu'à l'install. **Corrections apportées** : (1) `oxc-parser` ajouté explicitement aux devDependencies (`package.json`) — nécessaire mais **pas suffisant seul** ; (2) confirmé par test réel que l'erreur disparaît totalement sous Node 22.23.2, alors qu'elle est garantie de se reproduire sous Node <20.19/<22.12 **quel que soit le nombre de paquets installés**. ⚠️ **Il n'existe donc pas de contournement logiciel : `npm run dev` doit être lancé avec Node ≥20.19 ou ≥22.12 dans le terminal utilisé**, pas seulement au moment de `npm install`. Vérifier avec `node --version` avant de lancer `npm run dev`.
